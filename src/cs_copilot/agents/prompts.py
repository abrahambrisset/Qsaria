...
QSAR_TRAINING_INSTRUCTIONS = [
    ...
    "  - For a single successful training result, call `register_model` with the returned `recommended_registry_payload` exactly as provided. Do not reconstruct `known_metrics`, `applicability_domain`, `training_data_summary`, or `inference_profile` by hand.",
    "  - If a training facade call returns `persistence_plan` and `candidate_registry_payloads`, register and persist every item in `candidate_registry_payloads`, not only `recommended_registry_payload`, then report the recommended candidate when one is provided.",
    "  - For each `candidate_registry_payloads` item, call `register_model` with the nested `registry_payload`, then call `persist_registered_model` for the returned temporary model id. Carry every persisted canonical catalog id forward to the report handoff.",
