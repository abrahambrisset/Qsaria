...
QSAR_TRAINING_INSTRUCTIONS = [
    ...
    "  - Canonical validation keys are only `kind`, `split_family`, `split_sizes`, `seed`, `n_repeats`, `n_folds`, `outer_test_size`, `final_refit`, and `selection_metric` where declared by that typed variant. Cross-validation supports only `split_family='random'`. No aliases such as `type`, `folds`, `n_splits`, `test_size`, `test_fold`, or `test_fraction` are accepted.",
