...
QSAR_TRAINING_INSTRUCTIONS = [
    ...
    "  - When a user asks which training hyperparameters are available or tunable, call `describe_backend_hyperparameters` before proposing values. Only pass officially declared direct parameters in `extra_args`.",
    ...
    "  - If a training tool fails before returning a successful result, stop and report the blocking error. This is terminal for the current request: do not retry the same training tool with altered validation fields, folds, seeds, split type, test fields, backend, representation, or training facade unless the user explicitly asks for a retry. A LightGBM failure must never trigger Chemprop, TabICL, holdout, repeated holdout, or a modified CV fallback. Do not probe guessed output paths, guessed `test_predictions.csv` paths, or JSON summaries that were not returned by the failed tool call.",
    "  - In particular, never retry a failed `train_chemprop_model` call with omitted or inferred arguments. Preserve every explicit user constraint (including `hyperparameter_tuning.n_trials`) only when the user explicitly asks for a retry; otherwise report the failure.",
    ...
    "  - For a custom tuning request, pass `hyperparameter_tuning` (for example `{'n_trials': 20, 'parameters': ['n_estimators', 'max_depth']}`). Direct values in `extra_args` are fixed and excluded from LightGBM search.",
    "  - For LightGBM, understand explicit natural-language requests for a multivariate Optuna TPE sampler (for example `Optuna TPE multivarié`, `TPE multivarié`, or `Optuna multivarié`) and pass `hyperparameter_tuning={'engine': 'optuna_tpe_multivariate'}`. Do not infer this engine from an unqualified tuning request: standard training keeps `optuna_tpe`.",
    ...
    "  - If the user explicitly asks to remove every eligible/flagged outlier (for example `supprime tous les outliers éligibles`, `retire l'ensemble des molécules flaggées outliers`, `enlève tous les outliers`, or `remove all eligible outliers`), the training call MUST include exactly `outlier_analysis={'selection_fraction': 1.0}`. Do not leave this as a verbal intent or use the 10% default. This means every row satisfying the established regression eligibility rule is removed, not every validation row merely carrying an AC or AD flag.",
    "  - If the user explicitly asks not to run post-selection outlier removal, pass `outlier_analysis={'enabled': False}`. Do not infer either override from an unqualified training request.",
    ...
    "  - Use only this canonical validation vocabulary: `type` (`holdout`, `repeated_holdout`, `cross_validation`, `full_train`), `split_family` (`random`, `scaffold`, `cluster`; cross_validation currently only supports `random`), `split_sizes` (`[train, test]` or `[train, validation, test]`; never `[1.0,0.0]`), `n_folds`, `folds`, `n_splits`, `n_repeats`, `outer_test_size`, `final_refit`, and `selection_metric`. In a CV, `outer_test_size` is the only valid name for an isolated external test; never use `test_size`, `test_fold`, or `test_fraction`.",
    ...
    '  - Example for `cross-validation random a 3 folds avec un test externe de 10 %`: `validation_strategy={"type":"cross_validation","split_family":"random","n_folds":3,"outer_test_size":0.10,"final_refit":true}`.',
...
QSAR_REPORT_INSTRUCTIONS = [
    ...
    "  - Use `Resultats de test externe` only when `report_facts.evaluation.scope=external`; state that the isolated test data were not used for selection. Use `Resultats de test interne` when `scope=internal`; state explicitly that evidence is OOF/internal, no external test was used, and final refits are not independently evaluated.",
