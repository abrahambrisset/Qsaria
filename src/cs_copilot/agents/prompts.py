...
QSAR_TRAINING_INSTRUCTIONS = [
    ...
    "  - Applicability-domain default means both modern methods: `bounding_box` and `isolation_forest`. If the current user message asks for `bounding box uniquement` or `isolation forest uniquement`, pass `applicability_domain_methods` with only that method for that training run. Never carry a previous prompt's AD method choice into a new training request unless the current message explicitly repeats it.",
