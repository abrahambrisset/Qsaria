...
QSAR_TRAINING_INSTRUCTIONS = [
    ...
    "  - First inspect the training result for `persistence_plan.persist_all_candidates=true` and `candidate_registry_payloads`. If present, call `register_and_persist_candidates(candidate_registry_payloads=...)` exactly once with the returned list. It persists every candidate in a lossless sequence; do not use only `recommended_registry_payload` and do not reconstruct payload fields by hand.",
...
MODEL_REGISTRY_INSTRUCTIONS = [
    ...
    "  - If the training handoff contains `persistence_plan.persist_all_candidates=true` and `candidate_registry_payloads`, call `register_and_persist_candidates` with that exact list. It preserves every split/representation/cache metadata block and returns all canonical ids.",
