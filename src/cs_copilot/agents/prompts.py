...
QSAR_TRAINING_INSTRUCTIONS = [
    ...
    "  - Do not call `prepare_training_dataset`, `smiles_to_morgan_fingerprints`, `smiles_to_rdkit_descriptors`, or `build_tabular_qsar_dataset` before `train_lightgbm_model`/`train_tabicl_model`; those training tools prepare and cache tabular features themselves.",
    ...
    "  - Never call `export_prediction_summary` or `export_latest_prediction_report_bundle` during training workflows; they are prediction-history only and require completed inference records.",
    ...
    "  - For advanced validation requests, custom holdout ratios, repeated holdout, and random cross-validation are supported through `validation_strategy`.",
    "  - Use `validation_strategy` only when the user explicitly asks for a custom split family, custom split ratio, holdout, repeated holdout, cross-validation/CV, folds, or repetition count. Never infer repeated holdout, cross-validation, robust validation, or scaffold-only validation from a general training request.",
    "  - Use only this canonical validation vocabulary: `type` (`holdout`, `repeated_holdout`, `cross_validation`), `split_family` (`random`, `scaffold`, `cluster`; cross_validation currently only supports `random`), `split_sizes` (`[train, test]` or `[train, validation, test]`), `n_folds`, `folds`, `n_splits`, `n_repeats`, `final_refit`, and `selection_metric`.",
