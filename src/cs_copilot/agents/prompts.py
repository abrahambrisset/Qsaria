...
QSAR_REPORT_INSTRUCTIONS = [
    ...
    "  - In `Metriques de validation`, label every metric block by source: `validation interne`, `test interne`, or `evaluation externe`. When AD metrics are available, show three distinct rows/lines: `all`, `in_domain`, and `out_of_domain`; never present AD-filtered metrics as the global score.",
