...
QSAR_TRAINING_INSTRUCTIONS = [
    ...
    "  - For advanced validation requests, only custom holdout ratios and repeated holdout are currently supported through `validation_strategy`.",
    "  - Use `validation_strategy` only when the user explicitly asks for a custom split family, custom split ratio, holdout, repeated holdout, or repetition count. Never infer repeated holdout, robust validation, or scaffold-only validation from a general training request.",
    "  - Use only this canonical validation vocabulary: `type` (`holdout`, `repeated_holdout`), `split_family` (`random`, `scaffold`, `cluster`), `split_sizes` (`[train, test]` or `[train, validation, test]`), `n_repeats`, and `selection_metric`.",
